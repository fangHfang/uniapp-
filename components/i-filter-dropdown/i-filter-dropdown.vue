<template>
	<view class="i-filter-dropdown" :class="{'setDropdownBottom':maskVisibility}" :style="{'top':menuTop+'px'}"
	 @touchmove.stop.prevent="discard" @tap.stop="discard">
		...
		<view class="mask" :class="{'show':isShowMask,'hide':maskVisibility!=true}" @tap="togglePage(showPage,'mask')"></view>
			...
			<view class="sub-menu-class " :class="{'show':showPage==page_index,'hide':pageState[page_index]!=true}">
				<block v-if="page.type=='hierarchy'&& page.submenu.length>0">
					<view class="hierarchy-box">
						...
						<view class="btn-box">
							<view class="reset" @tap="resetHierarchyData">重置</view>
							<view class="submit" @tap="setHierarchyData">确定</view>
						</view>
					</view>
				</block>
		...
		methods: {
			...
			selectHierarchyMenu(page_index, level1_index, level2_index, level3_index) {
				...
				if (level1_index != null && level2_index == null && level3_index == null && this.shadowActiveMenuArr[page_index][0] ==
					...
					if(level2_index != null || this.activeMenuArr[page_index].length >= 2){
						if(this.activeMenuArr[page_index][1] === level2_index){
							this.activeMenuArr[page_index].splice(1, 1,null)
						}else{
							this.activeMenuArr[page_index].splice(1, 1,level2_index) || this.activeMenuArr[page_index].splice(1, 1);
						}
					}
				...
				if (level3_index != null || level2_index != null || (level1_index != null && this.subData[page_index].submenu[
					...
					console.log(this.shadowActiveMenuArr[page_index],'this.shadowActiveMenuArr[page_index]')
			...
			resetHierarchyData(){
				//UI状态
				this.activeMenuArr[2] = [null,null]
				this.shadowActiveMenuArr[2] = [null,null]
				//设定顶部菜单名字
				if (this.updateMenuName) {
					this.setMenuName();
				}
				setTimeout(()=>{
					this.togglePage(this.showPage,'mask')
					this.$emit('reset')
				},10)
			},
			//写入结果，筛选
			...
			resetFilterData(page_index) {
				...
				this.shadowActiveMenuArr[page_index] = [[]]
			...
			//重置结果和ui，筛选
			// 切换门店后重置请求用
			resetFilterDataByChangeStore(page_index) {
				let tmpArr = [];
				let level = this.shadowActiveMenuArr[page_index].length;
				while (level > 0) {
					tmpArr.push([]);
			...
			//菜单开关
			togglePage(index, tag) {
				if (index == this.showPage) {
					this.hidePageLayer(true, tag);
					this.hideMask();
					this.showPage = -1;
				} else {
					if (this.showPage > -1) {
						this.hidePageLayer(false, tag);
					}
...
<style lang="scss">
	...
	.sub-menu-class {
		...
		&.show {
			transform: translate3d(0, calc(60rpx + 1rpx), 0);
		}
	}
