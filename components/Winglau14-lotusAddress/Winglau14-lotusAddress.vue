<template>
	<!--地址picker-->
	<view :status="checkStatus" v-if="lotusAddressData.visible" style="position: relative;">
		<view :style="lotusMaskStyle"  class="lotus-address-mask"  @tap="cancelPicker"></view>
		<view :style="lotusBoxStyle" :class="lotusAddressData.visible?'lotus-address-box':'lotus-address-box lotus-address-box-out'">
			...
			<view class="lotus-address-picker-box">
				<!--省-->
				<scroll-view scroll-y :scroll-into-view="'pid'+pChoseIndex" class="lotus-address-picker-box-item" :style="pickerBoxItemStyle">
					<text @tap="clickPicker(0,pIndex,pItem);" :id="'pid'+pIndex" :class="pIndex === pChoseIndex?'lotus-address-picker lotus-address-picker2':'lotus-address-picker'"  v-for="(pItem,pIndex) in province" :key="pIndex">{{pItem}}</text>
				</scroll-view>
				<!--市-->
				<scroll-view scroll-y :scroll-into-view="'cid'+cChoseIndex" class="lotus-address-picker-box-item" :style="pickerBoxItemStyle">
					<text @tap="clickPicker(1,cIndex,cItem);" :id="'cid'+cIndex" :class="cIndex === cChoseIndex?'lotus-address-picker lotus-address-picker2':'lotus-address-picker'" v-for="(cItem,cIndex) in city" :key="cIndex">{{cItem}}</text>
				</scroll-view>
				<!--区-->
				<scroll-view scroll-y :scroll-into-view="'tid'+tChoseIndex" class="lotus-address-picker-box-item" :style="pickerBoxItemStyle">
					<text @tap="clickPicker(2,tIndex,tItem);" :id="'tid'+tIndex" :class="tIndex === tChoseIndex?'lotus-address-picker lotus-address-picker2':'lotus-address-picker'" v-for="(tItem,tIndex) in town" :key="tIndex">{{tItem}}</text>
				</scroll-view>
				<!--区END-->
			</view>
		</view>
	...
	export default {
		...
		computed:{
			...
			lotusBoxStyle(){
				...
				let paddingBottom=uni.getSystemInfoSync().safeAreaInsets.bottom
				return `width:${width}px;padding-bottom:${paddingBottom}px;`
			},
			// 宽高
			lotusMaskStyle(){
				let width=uni.getSystemInfoSync().windowWidth
