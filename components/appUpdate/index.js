import config from '@/api/system/system.js'
...
// 主颜色
const $mainColor = "ed6d00";

// 弹窗图标url
const $iconUrl = require('@/static/image/update/ic_ar.png');
...
const getServerNo = function(version,isPrompt = false, callback) {
	...
	config.getAppVersion(postData).then(({ data }) => {
		const { editionNo,updateNum,forceUpdate } = res.data
		if(plus.runtime.version === updateNum){
			return
		}
		let res = {
			versionCode: data.updateNum,
			versionName: data.updateNum,
			versionInfo: '修复漏洞',
...
// 是否强制更新弹窗
function forceUpdatePopup(data, callback) {
	// 弹窗遮罩层
	let maskLayer = new plus.nativeObj.View("maskLayer", { //先创建遮罩层
		top: '0px',
		left: '0px',
