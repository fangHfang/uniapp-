<template>
	<view v-if="isShow" class="picker" @touchmove.stop.prevent :class="pageInside?'page-inside':''">
		<!-- 日期选择器 -->
		...
		<view v-if="showTimePicker" class="picker">
			<view class="picker-modal picker-time">
				...
				<picker-view class="picker-modal-time" indicator-class="picker-modal-time-item" :value="timeValue" @change="onTimeChange">
					<picker-view-column>
						<view v-for="(v,i) in 24" :key="i">{{i}}时</view>
					</picker-view-column>
					<picker-view-column>
						<view v-for="(v,i) in 60" :key="i">{{i}}分</view>
					</picker-view-column>
					<picker-view-column v-if="showSeconds">
						<view v-for="(v,i) in 60" :key="i">{{i}}秒</view>
					</picker-view-column>
				</picker-view>
				<view class="picker-modal-footer">
					<view class="picker-modal-footer-info">
	...
	export default {
		...
		methods: {
			...
			setHighlightValue(value) {
				//将字符串解析为Date对象
				let parseDateStr = (str) => (this.format ? DateTools.inverse(str, this.format) : DateTools.parse(str));
				if(this.pageInside && this.disabledSelectDate){
					value.forEach((val)=>{
						this.checkeds.push(parseDateStr(val));
					})
			...
			procCalendar(item) {
				...
				//有两个日期
				if (this.checkeds.length == 2 && !this.pageInside) {
			...
			onSelectDate(date) {
				// 如果禁止点击选中日期
				if (this.disabledSelectDate) return
		...
		watch: {
			show(newValue, oldValue) {
				...
				newValue && this.setHighlightValue(this.highlightValue);
			...
			highlightValue:{
				handler(newValue, oldValue) {
					setTimeout(() => {
						this.setHighlightValue(newValue);
					}, 20);
				}
			}
		}
