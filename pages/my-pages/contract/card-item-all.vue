<template>
	<view class="product-item" @click="toDetail(cardItem.id)">
		<view class="item-head flex a-center j-space-between">
			...
			<view v-if="cardItem.scStoreAppDTO" class="item-time" :class="['yellow','green','red'][cardItem.scStoreAppDTO.approvalStatus]">{{['审核中','已签约','已拒绝'][cardItem.scStoreAppDTO.approvalStatus]}}</view>
			<view v-else class="item-time">未签约</view>
		</view>
		<view v-if="cardItem.scStoreAppDTO" class="item-con flex a-center j-space-between">
			<view class="item-contracted-volume">
				<view class="item-title">入库上限</view>
				<view class="item-number">{{cardItem.scStoreAppDTO.upperLimit}}</view>
			</view>
			<view class="item-contracted-volume">
				<view class="item-title">入库下限</view>
				<view class="item-contracted-number">{{cardItem.scStoreAppDTO.lowerLimit}}</view>
			</view>
		</view>
		<view class="item-foot flex a-center j-space-between">
			<view class="item-contract">
				<view class="item-contract-date">可签约时间：<text class="item-contract-color">{{cardItem.startDate}}~{{cardItem.endDate}}</text></view>
			</view>
